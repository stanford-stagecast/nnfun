...
void program_body()
{
  ...
  auto nn = make_unique<Network<float, batch_size, input_size, input_size, 1>>();
  ...
  for ( auto i = 0; i < 100; i ++ ) {
	/* test true function */
	  ...
	  nn->apply( input );
	  cout << "input: " << input << " => output: " << nn->output()(0, 0) << " => truth: " << ground_truth_output(0, 0) << endl;
	  ...
	  Matrix<float, input_size, 1> current_weights, current_weights_next;
	  for ( int j = 0; j < 16; j++ ) {
		current_weights( j ) = nn->layer0.weights()( j );
		current_weights_next( j ) = nn->next.weights()( j );
	  }
	  auto current_biase = nn->layer0.biases()( 0 );
	  auto current_biase_next = nn->next.biases()( 0 );

	  /* loss for 4/3 eta */
	  for ( int j = 0; j < 16; j++ ) {
		nn->layer0.weights()( j ) -= four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 0, j );
		nn->next.weights()( j ) -= four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 1, j );
	  }
	  nn->layer0.biases()( 0 ) -= four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 0, 16 );
	  nn->next.biases()( 0 ) -= four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 1, 16 );
	  nn->apply( input );
	  ...
	  for ( int j = 0; j < 16; j++ ) {
		...
		nn->next.weights()( j )
		  = current_weights_next( j ) - two_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 1, j );
	  }
	  nn->layer0.biases()( 0 )
		= current_biase - two_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 0, 16 );
	  nn->next.biases()( 0 )
		= current_biase_next - two_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 1, 16 );
	  nn->apply( input );
	  ...
	  if ( min_loss == current_loss ) {
		...
		for ( int j = 0; j < 16; j++ ) {
		  ...
		  nn->next.weights()( j ) = current_weights_next( j );
		}
		nn->layer0.biases()( 0 ) = current_biase;
		nn->next.biases()( 0 ) = current_biase_next;
		...
		for ( int j = 0; j < 16; j++ ) {
		  ...
		  nn->next.weights()( j )
			= current_weights_next( j ) - four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 1, j );
		}
		nn->layer0.biases()( 0 )
		  = current_biase - four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 0, 16 );
		nn->next.biases()( 0 )
		  = current_biase_next - four_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 1, 16 );
	  } else {
		for ( int j = 0; j < 16; j++ ) {
		  nn->layer0.weights()( j )
			= current_weights( j ) - two_third_lr * pd_loss_wrt_output * nn->getEvaluatedGradient( 0, j );
	  ...
	  cout << "weights layer 0: " << nn->layer0.weights() << endl;
	  cout << "weights layer 1: " << nn->next.weights() << endl;
	  cout << "biase layer 0: " << nn->layer0.biases()( 0 ) << endl;
	  cout << "biase layer 1: " << nn->next.biases()( 0 ) << endl;
	}
  ...
  for ( int i = 1; i < 100; i++ ) {
	...
	cout << "input: " << i << " output: " << nn->output()(0, 0) << " ground truth: " << true_function(i * 1.0) <<endl;
	// cout << i << endl;
  }
}
