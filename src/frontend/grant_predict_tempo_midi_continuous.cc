...
{
  float curr_time_secs = std::chrono::duration_cast<std::chrono::milliseconds>( std::chrono::steady_clock::now()
                                                                - base_time )
    .count()/1000.0;
  //cout << std::fixed << "curr_time: " << curr_time_secs << "\n";
  ...
  for ( auto i = 0; i < 16; i++ ) {
    if ( i >= num_notes ) {
      ...
      ret_mat( i ) = times[i] - curr_time_secs;

    }
  }
  cout << "RETURNED MATRIX: " << ret_mat << endl;
  return ret_mat;
}
...
void nn_input_from_piano( const string& midi_filename) {
    ...
    while ( event_loop->wait_next_event( 50 ) != EventLoop::Result::Exit ) {
      ...
      cout << "prediction: " <<  nn->get_output()( 0, 0 ) << endl;
    }
}
