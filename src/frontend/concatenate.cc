/**
 * File name: concatenated.cc
 * Last Update: July 14 2022
 * Description: This file tests the printWeights new features for concatenating
 *              the weights of two neural networks as one. It is an edited version
 *              of big_nn_1.cc
 *              This file uses a neural network (5 layers) that predicts
...
/* current attempt at decomposing concatenation */

// string concatenateWeights(string outfileName, auto nn1, auto nn2, int layer_offset) {
//   string weightsFile = "../src/frontend/" + outfileName;
//   ofstream ofs{weightsFile};
//   auto cout_buff = cout.rdbuf();
//   cout.rdbuf(ofs.rdbuf());
//   nn1->printWeights("nn1_weights.txt", 0, 0);
//   nn2->printWeights("nn2_weights.txt", 0, nn1_num_layers);
//   cout.rdbuf(cout_buff);
//   return "/home/jcoronad/nnfun/src/frontend/" + outfileName;
// }
...
//   // TRAINING FROM BOTH NEURAL NETWORKS SEPARATELY
  ...
  for ( int i = 0; i < iterations/2; i++ ) {
      ...
      if (i % ((iterations/2)/10000) == 0) {
        cout << (float(i)/(iterations/2))*100 << " percent done..." << endl;
      }
  }

  ofstream ofs{"../src/frontend/concatenated_weights.txt"};
  auto cout_buff = cout.rdbuf();
  cout.rdbuf(ofs.rdbuf());
  nn1->printWeights("nn1_weights.txt", 0, 0);
  nn2->printWeights("nn2_weights.txt", 0, 2);
  cout.rdbuf(cout_buff);
  ...
  // string weights_file = "/home/jcoronad/nnfun/src/frontend/concatenated_weights.txt";
