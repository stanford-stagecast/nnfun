...
void program_body( int x_boundary, float* main_w, float* main_b )
{
  ...
  double x_value = rand() % x_boundary;
  // while ( true ) {
  ...
  *main_w = nn->layer0.weights()( 0 );
  *main_b = nn->layer0.biases()( 0 );
...
int main( int argc, char*[] )
{
  try {
    ...
    float biase;
    for ( int i = 30; i < 60; i++ ) {
      program_body( i, &weight, &biase );
      cout << "x boundary: " << i << " , weight = " << weight << ", biase = " << biase << endl;
    }
