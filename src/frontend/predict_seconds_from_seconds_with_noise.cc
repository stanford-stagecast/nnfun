...
{
  auto nn = make_unique<
    NeuralNetwork<float, num_layers, batch_size, input_size, output_size, layer_size1, output_size>>();
 ...
 /******************** TESTING ********************/
  for (  int tempo = 30; tempo < 241; tempo++ ) {
    ...
    nn->apply(input);
    cout << tempo << " -> " << 60.0/nn->get_output()( 0, 0 ) << endl;
  }
  cout << endl;
