...
template<class T, unsigned int num_of_layers, unsigned int batch_size, unsigned int input_size, unsigned int output_size, unsigned int... rest>
class NeuralNetwork
{
...
public:
  ...
  const Matrix<T, batch_size, output_size>& get_output() {
    return nn->output();
  }
  ...
  void gradient_descent(Matrix<T, batch_size, input_size>& input, Matrix<T, batch_size, output_size>& ground_truth_output, float learning_rate) {
    nn->apply(input);
    nn->computeDeltas();
